...
class DagsMonitorPage(PageTemplate):
    ...
    def __init__(cls):
        ...
        cls._sidebar_filters = [
            {"col": "WORKLOAD_TYPE", "key": f"{os.path.basename(__file__)}._fltr_workload_type", "label": "Select workloads"},
            {"col": "RUN_MODE", "key": f"{os.path.basename(__file__)}._fltr_run_mode", "label": "Select run modes"},
            {"col": "WAREHOUSE_SIZE", "key": f"{os.path.basename(__file__)}._fltr_wh_size", "label": "Select warehouse sizes"},
            {"col": "DATA_FORMAT", "key": f"{os.path.basename(__file__)}._fltr_dta_fmt", "label": "Select data "}
        ]
    ...
    def display_as_metrics(cls) -> None:
        ...
        # Display 2 lines of metrics one the main kpi as Sim and Avg
        for type in ["Tot", "Avg"]:
